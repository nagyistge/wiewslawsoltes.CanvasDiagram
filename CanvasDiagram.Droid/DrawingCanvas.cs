...
namespace CanvasDiagram.Droid
{
	...
	public class DrawingCanvas : SurfaceView, ISurfaceHolderCallback
	{
		#region Properties
		...
		public DrawingCanvas (Context context, IAttributeSet attrs)
			: base(context, attrs)
		{
			Initialize (context);
		}
		...
		public void Start ()
		{
			try
			{
				if (thread == null)
				{
					thread = new DrawingThread (this.Holder, this);
		...
		private void FinishCurrentElement (float x, float y)
		{
		...
		private void SetPanStart (float x, float y)
		{
		...
		private void PinchToZoom (float x0, float y0, float x1, float y1)
		{
		...
		private void StartPinchToZoom (float x0, float y0, float x1, float y1)
		{
		...
		private void HandleOneInputDown (float x, float y)
		{
			float scaledX = (x - translate.X) / zoom;
			float scaledY = (y - translate.Y) / zoom;
			bool haveWire = false;
		...
		private void HandleOneInputMove (float x, float y)
		{
			if (currentElement != null) 
			{
